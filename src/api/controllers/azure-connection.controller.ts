...
export async function listAzureConnections(req: Request, res: Response) {
  const tenant = tenantId(req);
  if (!tenant) return res.status(403).json({ success: false, error: "Not assigned to an organization yet" });
  const data = await providerConnectionService.listForTenant(tenant, "azure");
  res.json({ success: true, data: data.map(({ encryptedCredential, ...rest }) => rest) });
}

/** Needs an Azure AD app registration (service principal) with Cost Management Reader on the subscription. */
export async function createAzureConnection(req: Request, res: Response) {
  const tenant = tenantId(req);
  if (!tenant) return res.status(403).json({ success: false, error: "Not assigned to an organization yet" });
  if (!req.user?.id) return res.status(401).json({ success: false, error: "Not authenticated" });

  const { subscriptionId, azureTenantId, clientId, clientSecret } = req.body ?? {};
  if (!subscriptionId || !azureTenantId || !clientId || !clientSecret) {
    return res.status(400).json({
      success: false,
      error: "subscriptionId, azureTenantId, clientId and clientSecret are required",
    });
  }
  ...
  try {
    const connection = await providerConnectionService.createConnection({
      ...
      credential: JSON.stringify({ tenantId: azureTenantId, clientId, clientSecret }),
...
export async function syncAzureNow(req: Request, res: Response) {
  const tenant = tenantId(req);
  if (!tenant) return res.status(403).json({ success: false, error: "Not assigned to an organization yet" });
  ...
  const accepted = requestSyncNow(connection.id, () => syncAzureGpuCost(connection));
  if (!accepted) return res.status(409).json({ success: false, error: "A sync for this connection is already running" });
  res.json({ success: true, message: "Sync started" });
}

export async function disconnectAzureConnection(req: Request, res: Response) {
  const tenant = tenantId(req);
  if (!tenant) return res.status(403).json({ success: false, error: "Not assigned to an organization yet" });
  const id = paramString(req.params.id);
  const connection = await providerConnectionService.getForTenant(tenant, id);
