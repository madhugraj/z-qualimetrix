import { CheckCircle2, Download, FileText, FolderSync, Link2, Search } from "lucide-react";
 ...
 * its documentation. Step 3 — browse and download. Connections are session
 * state only; swap the mock catalogue for the connector APIs later.
 */
export function DocumentHub() {
  const [project, setProject] = useState(PRODUCTS[0]);
  const [connected, setConnected] = useState<Record<string, DocSourceId[]>>({
    [PRODUCTS[0]]: ["gdrive", "confluence"],
  });
  ...
  const downloadAll = () => {
    ...
    exportText(
      ...
      docs.map(documentFileContent).join("\n\n---\n\n"),
    );
    toast.success(`${docs.length} documents bundled for ${project}`);
  };
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        {DOC_SOURCES.map((s) => {
          ...
          return (
            <div
              key={s.id}
              className="flex flex-col gap-2 rounded-2xl border border-glass-border/60 p-3"
            >
