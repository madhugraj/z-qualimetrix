...
export function CreateProductForm({ tenantId, onProductCreated, onCancel }: CreateProductFormProps) {
  ...
  const [jiraProjects, setJiraProjects] = useState<ExternalProject[]>([]);
  const [adoProjects, setAdoProjects] = useState<ExternalProject[]>([]);

  // Populate live project pickers only when each provider is actually connected —
  // falls back to plain free-text entry otherwise (progressive enhancement).
  useEffect(() => {
    ...
    loadProjectsIfConnected('jira', setJiraProjects);
    loadProjectsIfConnected('azure_devops', setAdoProjects);
  }, [tenantId]);
  ...
  // Jira scopes sync by the immutable project id (keys can be renamed/reassigned),
  // so picking a key from the connected site's live project list also captures its id.
  const handleJiraProjectKeyChange = (value: string) => {
    const match = jiraProjects.find(p => p.key === value);
    setFormData(prev => ({ ...prev, jiraProjectKey: value, jiraProjectId: match?.id ?? prev.jiraProjectId }));
    if (errors.jiraProjectKey) setErrors(prev => ({ ...prev, jiraProjectKey: undefined }));
  };
        ...
        <div className="border border-glass-border/60 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                id="jiraProjectKey"
                list="jira-project-options"
                value={formData.jiraProjectKey}
                onChange={(e) => handleJiraProjectKeyChange(e.target.value)}
                placeholder={jiraProjects.length ? "Pick a connected project or type a key" : "e.g., PROJ"}
                disabled={isLoading}
              />
              {jiraProjects.length > 0 && (
                <datalist id="jira-project-options">
                  {jiraProjects.map((p) => (
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                list="ado-project-options"
                value={formData.azureDevopsAreaPath}
                onChange={(e) => handleChange('azureDevopsAreaPath', e.target.value)}
                placeholder={adoProjects.length ? "Pick a project, optionally append \\Area" : "e.g., \\Project\\Area"}
                disabled={isLoading}
              />
              {adoProjects.length > 0 && (
                <datalist id="ado-project-options">
                  {adoProjects.map((p) => (
